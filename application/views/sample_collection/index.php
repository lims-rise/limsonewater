            ...
            <div class="col-xs-12">
                <div class="box box-black box-solid">
                    <div class="box-header">
                        <h3 class="box-title">Processing | Sample Collection </h3>
                    </div>
                    ...
                    <div class="box-body">
                        <div style="padding-bottom: 10px;">
                            <!-- <?php
                                    ...
                                    if ($lvl != 4){
                                        echo "<button class='btn btn-primary' id='addtombol'><i class='fa fa-wpforms' aria-hidden='true'></i> New Sample Collection</button>";
                                    }
                            ?>         -->
                            <?php echo anchor(site_url('Sample_collection/excel'), '<i class="fa fa-file-excel-o" aria-hidden="true"></i> Export to XLS', 'class="btn btn-success"'); ?>
                        </div>
                                ...
                                <table class="table ho table-bordered table-striped tbody" id="mytable" style="width:100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Barcode Sample Collection</th>
                                            <th>Lab Tech</th>
                                            <th>Sample Type</th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                    <div class="modal-header" style="background-color: #3c8dbc; color: white;">
                        ...
                        <h4 class="modal-title" id="modal-title">Sample Collection | New</h4>
                    </div>
                    <form id="formSample"  action= <?php echo site_url('Sample_collection/save') ?> method="post" class="form-horizontal">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="barcode_sample_collection" class="col-sm-4 control-label">Barcode Sample Collection</label>
                                <div class="col-sm-8">
                                    <input id="barcode_sample_collection" name="barcode_sample_collection" placeholder="Barcode Sample Collection" type="text" class="form-control" required>
                            ...
                            <div class="form-group">
                                <label for="date_processing" class="col-sm-4 control-label">Date of Processing</label>
                                <div class="col-sm-8">
                                    <input id="date_processing" name="date_processing" type="date" class="form-control" placeholder="Date of Processing" value="<?php echo date("Y-m-d"); ?>" max="<?php echo date('Y-m-d'); ?>">
                                </div>
                            </div>

                            <!-- <div class="form-group">
                                <label for="weight" class="col-sm-4 control-label">Try weight(g)</label>
                                <div class="col-sm-8">
                                    <input id="weight" name="weight" type="number" step="0.01"  class="form-control" placeholder="Try weight(g)" required>
                                </div>
                            </div> -->

                            <div class="form-group" id="weight_container" style="display: none;">
                                <label for="weight" class="col-sm-4 control-label">Weight(g)</label>
                                <div class="col-sm-8">
                                    <input id="weight" name="weight" type="number"  step="any" class="form-control" placeholder="Weight(g)">
                                </div>
                            </div>

                            <div class="form-group">
                                <label for="volume" class="col-sm-4 control-label">Volume(L)</label>
                                <div class="col-sm-8">
                                    <input id="volume" name="volume" type="number"  step="any"  class="form-control" placeholder="Volume(L)" required>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <input id="quantity" name="quantity" type="number"  step="any"  class="form-control" placeholder="Quantity" required>
                                </div>
                            </div>
                            ...
                            <div class="form-group" id="completed">
                                ...
                                <div class="col-sm-8">
                                    ...
                                    <label class="radio-inline">
                                        <input type="radio" name="completed" value="No" required> No
                                    </label>
                                </div>
                            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #dd4b39; color: white;">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="color: white;">&times;</button>
                <h4 class="modal-title"><i class="fa fa-trash"></i> Sample Collection | Delete <span id="my-another-cool-loader"></span></h4>
            </div>
            <div class="modal-body">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        if (barcodeFromUrl) {
            $('#barcode_sample_collection').val(barcodeFromUrl);
            $('#barcode_sample_collection').attr('readonly', true);
            $('#compose-modal').modal('show');
            handleSampleTypeInput('#sampletype');
            $('#mode').val('insert');
            $('#modal-title').html('<i class="fa fa-wpforms"></i> Sample Collection | New<span id="my-another-cool-loader"></span>');
        ...
        $(document).on('click', '.btn_delete', function() {
            let id = $(this).data('id');
            let url = '<?php echo site_url('Sample_collection/delete'); ?>/' + id;
        ...
        //     } else {
        //         $('#weight').attr('required', 'required'); // Tambahkan required jika sampletype adalah 'soil'
        //     }
        // });
        ...
        $('.idOneWaterSampleSelect').change(function() {
            ...
            if (id_one_water_sample) {
                $.ajax({
                    url: '<?php echo site_url('Sample_collection/getIdOneWaterDetails'); ?>', // URL untuk request AJAX
                    type: 'POST',
                    ...
                    success: function(response) {
                        ...
                        // Trigger input event to handle visibility of weight
        ...
        $('#id_one_water_sample').on("change", function() {
            ...
            $.ajax({
                type: "GET",
                url: "Sample_collection/barcode_restrict?id1="+id_one_water_sample,
                dataType: "json",
        ...
        $('#barcode_sample_collection').on("change", function() {
            let barcodeMoistureContent = $('#barcode_sample_collection').val();
            $.ajax({
                type: "GET",
                ...
                success: function(data) {
                    if (data.length == 1) {
                        ...
                        $('#barcode_sample_collection').focus();
                        $('#barcode_sample_collection').val('');       
                        $('#barcode_sample_collection').css({'background-color' : '#FFE6E7'});
                        setTimeout(function(){
                            $('#barcode_sample_collection').css({'background-color' : '#FFFFFF'});
                            setTimeout(function(){
                                $('#barcode_sample_collection').css({'background-color' : '#FFE6E7'});
                                setTimeout(function(){
                                    $('#barcode_sample_collection').css({'background-color' : '#FFFFFF'});
        ...
        table = $("#mytable").DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: {"url": "Sample_collection/json", "type": "POST"},
            columns: [
                // {
                //     "data": "barcode_sample",
                //     "orderable": false
                ...
                {"data": "barcode_sample_collection"},
                {"data": "initial"},
                {"data": "sampletype"},
                {"data": "date_processing"},
                {"data": "weight"},
                {"data": "volume"},
                {"data": "quantity"},
                {"data": "completed"},
        ...
        $('#mytable tbody').on('click', 'tr', function() {
            ...
            let rowId = rowData.id_sample_collection;
        ...
        $('#addtombol').click(function() {
            ...
            if (barcodeFromUrl) {
                $('#barcode_sample_collection').val(barcodeFromUrl).trigger('change');;
            } else {
                console.log('Barcode tidak ditemukan di URL');
            }
            handleSampleTypeInput('#sampletype');
        ...
        $('#mytable').on('click', '.btn_edit', function(){
            ...
            console.log(data);
            $('#mode').val('edit');
            $('#modal-title').html('<i class="fa fa-pencil-square"></i> Sample Collection | Update<span id="my-another-cool-loader"></span>');
            $('#idx_sample_collection').val(data.id_sample_collection);
            ...
            $('#date_processing').val(data.date_processing);
            ...
            $('#weight_container').val(data.weight_container);
            $('#volume').val(data.volume);
            $('#quantity').val(data.quantity);
            // Set radio button value
            if (data.completed === 'Yes') {
                $('#completed input[type="radio"][value="Yes"]').prop('checked', true);
            } else if (data.completed === 'No') {
                $('#completed input[type="radio"][value="No"]').prop('checked', true);
            }
            $('#comments').val(data.comments);
            $('#barcode_sample_collection').val(data.barcode_sample_collection).attr('readonly', true);
            // $('#barcode_sample_collection').attr('readonly', true);
            $('#compose-modal').modal('show');
        });  
