...
class Enterolert_hemoflow extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Enterolert_hemoflow_model');
    ...
    public function index()
    {
        ...
        $data['id_one'] = $this->Enterolert_hemoflow_model->getID_one();
        $data['sampletype'] = $this->Enterolert_hemoflow_model->getSampleType();
        $data['labtech'] = $this->Enterolert_hemoflow_model->getLabTech();
    ...
    public function json() {
        ...
        echo $this->Enterolert_hemoflow_model->json();
    }

    public function subjson() {
        $id = $this->input->get('id',TRUE);
        header('Content-Type: application/json');
        echo $this->Enterolert_hemoflow_model->subjson($id);
    }

    // public function subjson72() {
    //     $id = $this->input->get('id72',TRUE);
    //     header('Content-Type: application/json');
    //     echo $this->Enterolert_hemoflow_model->subjson72($id);
    // }
    ...
    public function read($id)
    {
        ...
        if (!$this->is_valid_return_url($return_url)) {
            $return_url = site_url("enterolert_hemoflow");
        }
    ...
    public function save() {
        ...
        if ($mode == "insert") {
            ...
            $this->Enterolert_hemoflow_model->insert($data);
    ...
    public function savedetail() {
            ...
            if($mode_det == "insert") {
                ...
                $insert_id = $this->Enterolert_hemoflow_model->insert_det($data);
                if ($insert_id) {
                    $this->session->set_flashdata('message', 'Create Record Success');
                } else {
            ...
            $redirect_url = site_url("enterolert_hemoflow/read/" . $id_one_water_sample);
            if ($this->is_valid_return_url($return_url)) {
    ...
    {
        $row = $this->Enterolert_hemoflow_model->get_by_id($id);
        $data = array(
            'flag' => 1,
            );
    ...
    public function delete_detail($id) 
    {
        ...
        if ($row) {
            ...
            $this->Enterolert_hemoflow_model->update_det($id, $data);
            $this->session->set_flashdata('message', 'Delete Record Success');
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
        }

        redirect(site_url('Enterolert_hemoflow/read/'.$id_parent));
    }
    ...
    public function getIdOneWaterDetails()
    {
        ...
        $oneWaterSample = $this->Enterolert_hemoflow_model->getOneWaterSampleById($idOneWaterSample);
        echo json_encode($oneWaterSample);
    }

    public function validate24() {
        $id = $this->input->get('id24');
        $data = $this->Enterolert_hemoflow_model->validate24($id);
        header('Content-Type: application/json');
    ...
    public function excel() {
        ...
        $moisture = $this->Enterolert_hemoflow_model->get_all();
        ...
        header('Content-Disposition: attachment;filename="Report_enterolert_hemoflow.xlsx"');
    ...
    public function validateEnterolertBarcode() {
        ...
        $data = $this->Enterolert_hemoflow_model->validateEnterolertBarcode($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    public function data_chart() 
    {
        ...
        $data = $this->Enterolert_hemoflow_model->data_chart($valueLargeWells, $valueSmallWells);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    public function barcode_restrict() 
    {
        ...
        $data = $this->Enterolert_hemoflow_model->barcode_restrict($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    public function saveReview()
    {
        ...
        $this->load->model('Enterolert_hemoflow_model');
        $this->Enterolert_hemoflow_model->updateSave($id, $data);
        echo json_encode([
            'status' => true,
    ...
    public function cancelReview()
    {
        ...
        $this->load->model('Enterolert_hemoflow_model');
        $updateResult = $this->Enterolert_hemoflow_model->updateCancel($id, $data);
