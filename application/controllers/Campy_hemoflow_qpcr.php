...
class Campy_hemoflow_qpcr extends CI_Controller
{
    ...
    {
        $data['test'] = $this->Campy_hemoflow_qpcr_model->getTest();
        $row = $this->Campy_hemoflow_qpcr_model->get_detail2($id);
        if ($row) {
            $data = array(
    ...
    public function save() {
        ...
        $id_campy_hemoflow_qpcr = $this->input->post('id_campy_hemoflow_qpcr', TRUE);
        $dt = new DateTime();
        ...
        if ($mode == "insert") {
            ...
            // die();
            $this->Campy_hemoflow_qpcr_model->delete_sample_volumes($id_campy_hemoflow_qpcr); // Hapus volume yang ada
            ...
            for ($i = 1; $i <= $number_of_tubes; $i++) {
                ...
                if ($volume) {
                    ...
                    // die();
                    $this->Campy_hemoflow_qpcr_model->insert_sample_volume($data_volume);
                }
            }
    ...
    public function getIdOneWaterDetails()
    {
        ...
        $oneWaterSample = $this->Campy_hemoflow_qpcr_model->getOneWaterSampleById($idOneWaterSample);
    ...
    public function validateCampyAssayBarcode() {
        ...
        $data = $this->Campy_hemoflow_qpcr_model->validateCampyAssayBarcode($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
