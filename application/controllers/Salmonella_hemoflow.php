...
class Salmonella_hemoflow extends MY_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Salmonella_hemoflow_model');
    ...
    public function index()
    {
        ...
        $data['labtech'] = $this->Salmonella_hemoflow_model->getLabTech();
        $data['tubes'] = $this->Salmonella_hemoflow_model->getTubes();
        $this->load_template_with_announcements('template', 'salmonella_hemoflow/index', $data);
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->Salmonella_hemoflow_model->json();
    }

    public function subjsonXld() {
        $id = $this->input->get('idXld',TRUE);
        header('Content-Type: application/json');
        echo $this->Salmonella_hemoflow_model->subjsonXld($id);
    }

    public function subjsonChromagar() {
        $id = $this->input->get('idChromagar',TRUE);
        header('Content-Type: application/json');
        echo $this->Salmonella_hemoflow_model->subjsonChromagar($id);
    }
    ...
    public function subjsonBiochemical() {
        ...
        header('Content-Type: application/json');
        echo $this->Salmonella_hemoflow_model->subjsonBiochemical($id, $biochemical_tube);
    }
    ...
    public function read($id)
    {
        ...
        if ($row) {
            ...
            // var_dump($data);
            // die();
            $this->template->load('template','salmonella_hemoflow/index_det', $data);
    ...
    {
        $data['test'] = $this->Salmonella_hemoflow_model->getTest();
        $row = $this->Salmonella_hemoflow_model->get_detail2($id);
        if ($row) {
            $data = array(
                'id_project' => $row->id_project,
                'id_sample' => $row->id_sample,
                'sample_description' => $row->sample_description,
                'test' => $this->Salmonella_hemoflow_model->getTest(),
                );
                $this->template->load('template','salmonella_hemoflow/index_det2', $data);
        }
        else {
    ...
    public function save() {
        ...
        $id_salmonella_hemoflow = $this->input->post('id_salmonella_hemoflow', TRUE);
        $dt = new DateTime();
        ...
        if ($mode == "insert") {
            ...
            $assay_id = $this->Salmonella_hemoflow_model->insert($data);
            ...
            for ($i = 1; $i <= $number_of_tubes; $i++) {
                ...
                if ($volume) {
                    $this->Salmonella_hemoflow_model->insert_sample_volume(array(
                        'id_salmonella_hemoflow' => $assay_id,
                        'tube_number' => $i,
            ...
            $this->Salmonella_hemoflow_model->updateSalmonellaHemoflow($id_salmonella_hemoflow, $data);

            // Update sample volumes
            $number_of_tubes = $this->input->post('number_of_tubes1', TRUE);
            // var_dump($number_of_tubes); // var dump jumlah tube
            // die();
            $this->Salmonella_hemoflow_model->delete_sample_volumes($id_salmonella_hemoflow); // Hapus volume yang ada
            ...
            for ($i = 1; $i <= $number_of_tubes; $i++) {
                ...
                if ($volume) {
                    ...
                    // die();
                    $this->Salmonella_hemoflow_model->insert_sample_volume($data_volume);
                }
            }
        ...
        redirect(site_url("salmonella_hemoflow"));
    }
    ...
    public function saveResultsXld() {
        ...
        if ($mode == "insert") {
            ...
            $assay_id = $this->Salmonella_hemoflow_model->insertResultsXld($data);
            ...
            $this->autoSaveChromagarIfAllZero($id_salmonella_hemoflow, $number_of_tubes, $date_sample_processed, $time_sample_processed);
            ...
            for ($i = 1; $i <= $number_of_tubes; $i++) {
                ...
                if ($plate !== null) {
                    ...
                    $this->Salmonella_hemoflow_model->insert_black_colony_plate_Chromagar($data_plate);
                }
            }
    ...
    public function saveResultsChromagar() {
        ...
        if ($mode == "insert") {
            ...
            $assay_id = $this->Salmonella_hemoflow_model->insertResultsChromagar($data);
            ...
            for ($i = 1; $i <= $number_of_tubes; $i++) {
                ...
                if ($plate !== null) {
                    ...
                    $this->Salmonella_hemoflow_model->insert_purple_colony_plate($data_plate);
                }
            }
    ...
    private function autoGenerateBiochemicalResults($id_salmonella_hemoflow, $id_salmonella_hemoflow_result_chromagar, $number_of_tubes) {
        ...
        for ($tube = 1; $tube <= $number_of_tubes; $tube++) {
            ...
            if (!$existing) {
                ...
                $this->Salmonella_hemoflow_model->insertResultsBiochemical($data);
            } else {
     ...
     * Auto-update biochemical results when XLD data changes
     */
    private function autoUpdateBiochemicalFromXldChange($id_salmonella_hemoflow, $number_of_tubes) {
        $dt = new DateTime();
        
        // Get all ChroMagar results for this sample
        $chromagar_results_list = $this->Salmonella_hemoflow_model->getAllChroMagarResults($id_salmonella_hemoflow);
    ...
    /**
     * Auto-save ChroMagar results if all XLD results are 0
     */
    private function autoSaveChromagarIfAllZero($id_salmonella_hemoflow, $number_of_tubes, $date_sample_processed, $time_sample_processed) {
        $dt = new DateTime();
        ...
        if ($all_zero) {
            ...
            $existing_chromagar = $this->Salmonella_hemoflow_model->getAllChroMagarResults($id_salmonella_hemoflow);
            ...
            if (empty($existing_chromagar)) {
                ...
                $chromagar_id = $this->Salmonella_hemoflow_model->insertResultsChromagar($chromagar_data);
                ...
                for ($i = 1; $i <= $number_of_tubes; $i++) {
                    ...
                    $this->Salmonella_hemoflow_model->insert_purple_colony_plate($plate_data);
                }
                
                // Auto-generate biochemical results
                $this->autoGenerateBiochemicalResults($id_salmonella_hemoflow, $chromagar_id, $number_of_tubes);
    ...
    // }
        ...
        if (!$mode || !$id_salmonella_hemoflow || !$confirmation || !$biochemical_tube) {
            if ($is_ajax) {
                echo json_encode([
                    'status' => 'error',
        ...
        try {
            if ($mode == "insert") {
                ...
                $result = $this->Salmonella_hemoflow_model->insertResultsBiochemical($data);
    ...
    public function getIdOneWaterDetails()
    {
        ...
        $oneWaterSample = $this->Salmonella_hemoflow_model->getOneWaterSampleById($idOneWaterSample);
    ...
    public function excel($id) {
        ...
        $finalConcentration = $this->Salmonella_hemoflow_model->get_export($id);
        ...
        header('Content-Disposition: attachment;filename="Report_Salmonella_Hemoflow_Final_Concentrations.xlsx"');
    ...
    public function excel_all() {
        $spreadsheet = new Spreadsheet();
        $finalConcentration = $this->Salmonella_hemoflow_model->get_all_export();
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment;filename="Report_All_Salmonella_Hemoflow_Final_Concentrations.xlsx"');
        header('Cache-Control: max-age=0');
    ...
    public function validateSalmonellaAssayBarcode() {
        ...
        $data = $this->Salmonella_hemoflow_model->validateSalmonellaAssayBarcode($id);
        header('Content-Type: application/json');
    ...
    public function barcode_restrict() 
    {
        ...
        $data = $this->Salmonella_hemoflow_model->barcode_restrict($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    public function saveReview()
    {
        ...
        try {
            $this->Salmonella_hemoflow_model->update_salmonella_hemoflow($id, $data);
            echo json_encode([
                'status' => true,
    ...
    public function cancelReview()
    {
        ...
        $this->load->model('Salmonella_hemoflow_model');
        $updateResult = $this->Salmonella_hemoflow_model->updateCancel($id, $data);
    ...
    public function getCalculateMPN() {
        ...
        $id_salmonella_hemoflow = $this->input->get('id_salmonella_hemoflow', TRUE);

        if (!$id_salmonella_hemoflow) {
            echo json_encode([
                'status' => 'error',
                'message' => 'ID Salmonella Hemoflow is required.'
            ]);
            return;
        }
        
        try {
            $mpn_data = $this->Salmonella_hemoflow_model->get_calculate_mpn_by_salmonella_hemoflow($id_salmonella_hemoflow);
    ...
    public function saveCalculateMPN() {
        ...
        try {
            ...
            $id_salmonella_hemoflow = $this->input->post('id_salmonella_hemoflow_mpn', TRUE);
            ...
            if (!$id_salmonella_hemoflow || !$mpn_concentration || !$upper_ci || !$lower_ci) {
                echo json_encode([
                    'status' => 'error',
            ...
            if ($mode === 'insert') {
                ...
                $insert_id = $this->Salmonella_hemoflow_model->insertCalculateMPN($data);
    ...
    public function checkTubeExists() {
        try {
            $id_salmonella_hemoflow = $this->input->post('id_salmonella_hemoflow');
            $biochemical_tube = $this->input->post('biochemical_tube');
            
            if (!$id_salmonella_hemoflow || !$biochemical_tube) {
                echo json_encode([
                    'status' => 'error',
            ...
            $exists = $this->Salmonella_hemoflow_model->checkTubeExists($id_salmonella_hemoflow, $biochemical_tube);
    ...
    public function checkTubeNeedsUpdate() {
        ...
        try {
            ...
            if (!$id_salmonella_hemoflow || !$biochemical_tube || !$expected_confirmation) {
                echo json_encode([
                    'status' => 'error',
                    'message' => 'Missing required parameters.',
                    'exists' => false,
            ...
            $result = $this->Salmonella_hemoflow_model->checkTubeNeedsUpdate($id_salmonella_hemoflow, $biochemical_tube, $expected_confirmation);
    ...
    public function checkAnyTubeExists() {
        try {
            ...
            $hasData = $this->Salmonella_hemoflow_model->checkAnyTubeExists($id_salmonella_hemoflow);
