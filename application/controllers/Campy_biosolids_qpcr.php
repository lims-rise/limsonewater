...
class Campy_biosolids_qpcr extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Campy_biosolids_qpcr_model');
    ...
    {
        $data['id_one'] = $this->Campy_biosolids_qpcr_model->getID_one();
    ...
    public function read($id)
    {
        ...
        if ($row) {
            $data = array(
                ...
                'id_campy_biosolids_qpcr' => $row->id_campy_biosolids_qpcr,
    ...
    {
        $data['test'] = $this->Campy_biosolids_qpcr_model->getTest();
        $row = $this->Campy_biosolids_qpcr_model->get_detail2($id);
        if ($row) {
            $data = array(
                'id_project' => $row->id_project,
                'id_sample' => $row->id_sample,
                'sample_description' => $row->sample_description,
                'test' => $this->Campy_biosolids_qpcr_model->getTest(),
                );
                $this->template->load('template','campy_biosolids_qpcr/index_det2', $data);
        }
        else {
    ...
    public function save() {
        ...
        $id_campy_biosolids_qpcr = $this->input->post('id_campy_biosolids_qpcr', TRUE);
        $dt = new DateTime();
        ...
        if ($mode == "insert") {
            ...
            $assay_id = $this->Campy_biosolids_qpcr_model->insert($data);
            ...
            for ($i = 1; $i <= $number_of_tubes; $i++) {
                ...
                if ($volume) {
                    $this->Campy_biosolids_qpcr_model->insert_sample_volume(array(
                        'id_campy_biosolids_qpcr' => $assay_id,
                        'tube_number' => $i,
            ...
            // die();
            $this->Campy_biosolids_qpcr_model->delete_sample_volumes($id_campy_biosolids_qpcr); // Hapus volume yang ada
            ...
            for ($i = 1; $i <= $number_of_tubes; $i++) {
                ...
                if ($volume) {
                    ...
                    // die();
                    $this->Campy_biosolids_qpcr_model->insert_sample_volume($data_volume);
                }
            }
    ...
    //                 $this->Campy_biosolids_qpcr_model->insert_growth_plate(array(
    ...
    //         // Update sample volumes
    //         $number_of_tubes = $this->input->post('number_of_tubes1', TRUE);
    //         $this->Campy_biosolids_qpcr_model->delete_growth_plates($id_result_qpcr); // Hapus volume yang ada
    ...
    //                     'date_created' => $dt->format('Y-m-d H:i:s'),
    //                 );
    //                 $this->Campy_biosolids_qpcr_model->insert_growth_plate($data_plate);
    //             }
    //         }
    ...
    public function getIdOneWaterDetails()
    {
        ...
        $oneWaterSample = $this->Campy_biosolids_qpcr_model->getOneWaterSampleById($idOneWaterSample);
    ...
    public function validateCampyAssayBarcode() {
        ...
        $data = $this->Campy_biosolids_qpcr_model->validateCampyAssayBarcode($id);
        header('Content-Type: application/json');
        echo json_encode($data);
    }
