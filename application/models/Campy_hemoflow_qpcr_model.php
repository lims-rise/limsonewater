...
class Campy_hemoflow_qpcr_model extends CI_Model
{
    ...
    function json() {
        ...
        if ($lvl == 4){
            $this->datatables->add_column('action', anchor(site_url('campy_hemoflow_qpcr/read/$1'),'<i class="fa fa-th-list" aria-hidden="true"></i>', array('class' => 'btn btn-warning btn-sm')), 'id_campy_biosolids_qpcr');
        }
        else if ($lvl == 3){
            ...
            ".'<button type="button" class="btn_deleteCampyHemoflow btn btn-danger btn-sm" data-id="$1" aria-hidden="true"><i class="fa fa-trash-o" aria-hidden="true"></i></button>', 'id_campy_hemoflow_qpcr');
        }
